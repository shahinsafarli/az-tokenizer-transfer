...
| Trainer-side best metric | disabled (`null`) |
...
**Not repaired here, and not repairable in code.** The 22 pre-guard test
evaluations remain in `results/test_evaluation_ledger.jsonl`; whether those
observations influenced design decisions is a question about the project's
history, and the honest response is disclosure in the paper, not a code change.
