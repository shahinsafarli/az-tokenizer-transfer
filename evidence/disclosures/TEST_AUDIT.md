...
Exactly **22 model test-evaluation calls have been performed to date**. All are
retained in `results/test_evaluation_ledger.jsonl`: 5 current diagnostic files
under `results/runs/`, 12 historical transplant-control files, 4 files under
`results/pre_amendment/runs/`, and 1 stability-diagnostic file. These are spent
test measurements and are not deleted or reset.
...
- `src.analysis.errors` reads predictions already present in spent result files.
